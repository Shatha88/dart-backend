...
addContactResponse(Request request) async {
  try {
  ...
  final result = await supaEnv.from("users").select("id").eq("id_auth" , jwt.payload["sub"]);
  ...
  final contact = await supaEnv.from("contact").upsert({"id_user" : iduser, ...body});
  ...
  // return Response.ok("create contact successesed fro id : ${iduser.toString()} \n ${json.encode(body) }" ,headers: {"content-type": "application/json"});
    ...
    return CustomResponse().successResponse(
      ...
      msg: "create contact successesed fro id : ${iduser.toString()}",
      data: [json.encode(body)], statusCode: 201,
    );
